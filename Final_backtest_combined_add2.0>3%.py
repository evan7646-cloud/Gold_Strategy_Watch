...
def run_strategy(tf_file, ma_period, tf_name, df_daily, is_long_only=True, boost_units=2.0, alpha_thresh=0.03):  # 執行策略回測 (含動態 2.0 Units 加倉)
...
def compute_stats(trades, label):  # 從交易清單計算績效指標
    if not trades:  # 無交易
...
def main():  # 主程式進入點
    ...
    # 執行回測 (Alpha10 > 3% 加倉 2.0 units)
    trades_long = run_strategy('comex_gc1!_8h.csv', 30, '8H', df_daily, is_long_only=True, boost_units=2.0, alpha_thresh=0.03)  # 多單
    trades_short = run_strategy('comex_gc1!_8h.csv', 30, '8H', df_daily, is_long_only=False, boost_units=2.0, alpha_thresh=0.03)  # 空單
    ...
    rows = [  # 印出指標列
        ...
        ('2.0 Units 加倉觸發次數', 'boosted_count'),
    ...
    ax.plot(pd.to_datetime(df_a['exit_date']), df_a['pnl'].cumsum(), label='Combined (Alpha10>3% 2.0x)', color='#9c27b0', linewidth=2.0)  # 合併
    ...
    ax.set_title('Hybrid Gold Strategy — Cumulative PnL (Alpha10 > 3% Pyramid 2.0 Units)', fontsize=14, fontweight='bold')  # 標題
    ...
    fig.savefig('backtest_combined_add2.0_gt_3pct_comparison.png', dpi=300)  # 儲存 PNG 績效圖
    plt.close(fig)  # 關閉
    print("\n績效對比圖已儲存至 backtest_combined_add2.0_gt_3pct_comparison.png")  # 提示

    pd.DataFrame(all_trades).to_csv('all_trades_detail_add2.0_gt_3pct.csv', index=False)  # 儲存交易明細
    print("逐筆交易明細已儲存至 all_trades_detail_add2.0_gt_3pct.csv")  # 提示

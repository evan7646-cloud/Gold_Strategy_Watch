...
def run_backtest_simulation(df, ma_col='ma', atr_col='atr'): # 定義通用雙向交易回測引擎
    ...
    # 年化 Sharpe Ratio (假設無風險利率 0%, 一年 252 交易日)
    if std_daily_pnl > 0:
        sharpe_ratio = (mean_daily_pnl / std_daily_pnl) * np.sqrt(252) # 夏普比率公式
    else:
        sharpe_ratio = 0.0
        
    # 計算 Calmar Ratio (年化收益率 / 最大回撤)
    # 回測總天數
...
def main(): # 主流程函數
    print("🚀 開始執行 4H vs 8H 包括 Sharpe Ratio & Calmar Ratio 完整比較...") # 提示訊息
    
    tv = TvDatafeed() # 初始化 TradingView
    df_gold_1h = tv.get_hist(symbol='GC1!', exchange='COMEX', interval=Interval.in_1_hour, n_bars=10000) # 抓取 1H K線
    if df_gold_1h is None or df_gold_1h.empty: # 降級處理
        print("⚠️ 無法取得最新 1H K 線，改用本地 CSV 資料...") # 提示訊息
        df_gold_1h = pd.read_csv('comex_gc1!_8h.csv') # Local fallback
    ...
    print("\n================ 📊 完整策略指標比較排行榜 (依 Sharpe Ratio 排序) ================")
    print(res_df.to_string(index=False)) # 印出完整結果表格
    res_df.to_csv('backtest_4h_8h_full_metrics.csv', index=False) # 儲存完整比較 CSV
    print("\n✅ 完整風險收益指標比較已匯出至 backtest_4h_8h_full_metrics.csv")

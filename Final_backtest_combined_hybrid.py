...
def run_strategy(tf_file, ma_period, tf_name, df_daily, is_long_only=True, boost_units=2.0, alpha_thresh=0.03):  # 執行策略回測 (Alpha10 > 3% 時加倉 2.0 Units)
            ...
            if has_pos:  # 持有多單
                ...
                if t_close < main_pos['stop_price'] or not is_long_sig:  # 止損或信號轉空
                    for p in active_positions:  # 平倉所有部位
                        ...
                        raw_pnl = next_open - p['entry_price']  # 原始平倉損益
                    ...
                    if has_pyr:  # 已有加多
                        ...
                        if dy_close > dy_ma50 and dy_pyramid_long:  # 滿足牛市及加多條件
                            pyr_units = boost_units if dy_a10 > alpha_thresh else 1.0  # 觸發時若 Alpha10 > 3% 則賦予 2.0 units
...
def compute_stats(trades, label):  # 從交易清單計算績效指標與最大回撤點數
    ...
    stats = {  # 指標字典
        ...
        'boosted_count': (df_t['units'] == 2.0).sum()  # 2.0 Units 加倉次數
    }
...
def main():  # 主程式進入點
    ...
    # 執行回測 (Alpha10 > 3% 時加倉 2.0 units)
    trades_long = run_strategy('comex_gc1!_8h.csv', 30, '8H', df_daily, is_long_only=True, boost_units=2.0, alpha_thresh=0.03)  # 多單回測
    trades_short = run_strategy('comex_gc1!_8h.csv', 30, '8H', df_daily, is_long_only=False, boost_units=2.0, alpha_thresh=0.03)  # 空單回測
    ...
    print("=== 混合組合策略 (Alpha10 > 3% 加倉 2.0 Units) 績效結果 ===")  # 印出標題
    header = f"{'指標':<28} | {'8H 30MA Long-Only':>20} | {'8H 30MA Short-Only':>20} | {'Combined Portfolio':>20}"  # 表頭
    print(header)  # 表頭
    print("-" * len(header))  # 分割線

    rows = [  # 印出指標列
        ('累積損益 (單位點數)',      'total_pnl_pts'),  # 累積損益
        ('原始損益 (未扣成本)',     'total_raw_pts'),  # 原始損益
        ('2.0 Units 加倉觸發次數',  'boosted_count'),  # 2.0x 加倉次數
        ('最大回撤 (點數 pts)',     'max_dd_pts'),  # 最大回撤點數
        ('最大回撤 (百分比 %)',     'max_dd_pct'),  # 最大回撤百分比
    ...
    ax.plot(pd.to_datetime(df_a['exit_date']), df_a['pnl'].cumsum(), label='Combined Portfolio (Alpha10>3% 2.0x)', color='#9c27b0', linewidth=2.0)  # 合併
    ...
    ax.set_title('Hybrid Gold Strategy — Per-Trade Cumulative PnL (Alpha10 > 3% Pyramid 2.0 Units)', fontsize=14, fontweight='bold')  # 標題
    ...
    df_detail['drawdown_from_peak_pts'] = df_detail['running_peak'] - df_detail['cum_pnl']  # 距波峰回撤點數
    df_detail['max_drawdown_pts_so_far'] = df_detail['drawdown_from_peak_pts'].cummax()  # 歷史最大回撤點數

    detail_cols = [  # 選擇輸出欄位
        'trade_no', 'type', 'is_pyramid', 'units', 'entry_date', 'exit_date',
    ...
    print("逐筆交易明細已儲存至 all_trades_detail.csv")  # 成功提示

    # ---- 最終核心數字快速確認 ----
    print(f"\n{'='*60}")  # 分割線
    print(f"✅ 正確回測結果 (Alpha10 > 3% 時加倉 2.0 Units)")  # 標題

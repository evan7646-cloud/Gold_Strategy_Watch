...
def calculate_atr(df, period=14):  # 計算 ATR
    high = df['high']  # 取得當期最高價
    low = df['low']  # 取得當期最低價
    close_prev = df['close'].shift(1)  # 取得前一期收盤價
    tr1 = high - low  # 計算當期高低差
    tr2 = (high - close_prev).abs()  # 最高與前收絕對差
    tr3 = (low - close_prev).abs()  # 最低與前收絕對差
    tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)  # 取三者最大值 TR
    return tr.rolling(period).mean()  # 回傳 14 期 ATR 均值
...
def run_strategy_sizing(df, is_long_only=True, alpha_threshold=None, boost_units=2.0, alpha_col='daily_alpha10_avail'):  # 執行動態倉位測試
    n = len(df)  # 資料筆數
    ...
    for i in range(n - 1):  # 遍歷 K 線
        ...
        alpha_val = df.loc[i, alpha_col]  # 當前 Alpha 值
        ...
        if is_long_only:  # 多單邏輯
            if has_pos:  # 已持有部位
                ...
                has_pyr = len(pyr_pos) > 0  # 是否已有加倉

                if t_close < main_pos['stop_price'] or not is_long_sig:  # 主部位清倉條件
                    for p in active_positions:  # 結算所有部位
                        holding_days = (next_date.date() - p['entry_date'].date()).days  # 持有天數
                        raw_pnl = next_open - p['entry_price']  # 點數損益
                        net_pnl_pts = raw_pnl - 0.3 - (holding_days * 0.75)  # 扣點差過夜費（每單位點數）
                        weighted_pnl = net_pnl_pts * p['units']  # 加權淨損益
                        completed_trades.append({'type': 'Long', 'pnl': weighted_pnl, 'pts': net_pnl_pts, 'units': p['units'], 'exit_date': next_date})  # 記錄
                    ...
                    if has_pyr:  # 已有加倉
                        ...
                        if t_close < p_pos['stop_price']:  # 加倉部位單獨止損
                            ...
                            raw_pnl = next_open - p_pos['entry_price']  # 點數損益
                            net_pnl_pts = raw_pnl - 0.3 - (holding_days * 0.75)  # 扣點差過夜費
                            weighted_pnl = net_pnl_pts * p_pos['units']  # 加權淨損益
                            completed_trades.append({'type': 'Long', 'pnl': weighted_pnl, 'pts': net_pnl_pts, 'units': p_pos['units'], 'exit_date': next_date})  # 記錄
                        else:  # 保留加倉
                            if t_close > t_prev_high:  # 移動止損
                                p_pos['stop_price'] = min(t_low, t_prev_low) - 1 * t_atr  # 更新加倉止損
                            new_active.append(p_pos)  # 保留加倉
                    else:  # 未加倉 -> 檢查是否滿足加倉
                        if dy_close > dy_ma50 and dy_pyramid_long:  # 滿足基本加多
                            # 決定加倉手數 (units)
                            pyr_units = 1.0  # 預設手數
                            if alpha_threshold is not None and alpha_val > alpha_threshold:  # 若 Alpha 達到強勢門檻
                                pyr_units = boost_units  # 提升手數至 boost_units

                            new_active.append({  # 建立加多部位
                                'type': 'Long', 'is_pyramid': True, 'units': pyr_units,
                                'entry_date': next_date, 'entry_price': next_open,
                                'stop_price': min(t_low, t_prev_low) - 1 * t_atr  # 加多止損價
                            })  # 加多部位結束
            ...
            if has_pos:  # 持有空單
                ...
                if t_close > main_pos['stop_price'] or is_long_sig:  # 主部位清倉條件
                    for p in active_positions:  # 結算所有部位
                        holding_days = (next_date.date() - p['entry_date'].date()).days  # 持有天數
                        raw_pnl = p['entry_price'] - next_open  # 點數損益
                        net_pnl_pts = raw_pnl - 0.3 + (holding_days * 0.27)  # 扣點差加利息
                        weighted_pnl = net_pnl_pts * p['units']  # 加權淨損益
                        completed_trades.append({'type': 'Short', 'pnl': weighted_pnl, 'pts': net_pnl_pts, 'units': p['units'], 'exit_date': next_date})  # 記錄
                    active_positions = []  # 清空持倉
                else:  # 保留並檢查加倉
                    ...
                    if has_pyr:  # 已有加倉
                        ...
                        if t_close > p_pos['stop_price']:  # 加倉部位單獨止損
                            holding_days = (next_date.date() - p_pos['entry_date'].date()).days  # 持有天數
                            raw_pnl = p_pos['entry_price'] - next_open  # 點數損益
                            net_pnl_pts = raw_pnl - 0.3 + (holding_days * 0.27)  # 扣點差加利息
                            weighted_pnl = net_pnl_pts * p_pos['units']  # 加權淨損益
                            completed_trades.append({'type': 'Short', 'pnl': weighted_pnl, 'pts': net_pnl_pts, 'units': p_pos['units'], 'exit_date': next_date})  # 記錄
                        else:  # 保留加倉
                            if t_close < t_prev_low:  # 移動止損
                                p_pos['stop_price'] = max(t_high, t_prev_high) + 1 * t_atr  # 更新加倉止損
                            new_active.append(p_pos)  # 保留加倉
                    else:  # 未加倉 -> 檢查是否滿足加倉
                        if dy_close < dy_ma50 and dy_pyramid_short:  # 滿足基本加空
                            pyr_units = 1.0  # 預設手數
                            if alpha_threshold is not None and alpha_val < -alpha_threshold:  # 若 Alpha 負向超過門檻
                                pyr_units = boost_units  # 提升手數至 boost_units

                            new_active.append({  # 建立加空部位
    ...
    if len(active_positions) > 0:  # 未平倉強制結算
        ...
        for p in active_positions:  # 遍歷部位
            ...
            net_pnl_pts = raw_pnl - 0.3 - (holding_days * ov_fee)  # 每單位淨損益
            weighted_pnl = net_pnl_pts * p['units']  # 加權淨損益
            completed_trades.append({'type': p['type'], 'pnl': weighted_pnl, 'pts': net_pnl_pts, 'units': p['units'], 'exit_date': last_date})  # 記錄

    return completed_trades  # 回傳交易紀錄

def eval_trades(trades):  # 計算指標摘要
    if not trades: return {'total_pnl': 0, 'max_dd': 0, 'sharpe': 0, 'trades': 0, 'win_rate': '0%', 'boosted_count': 0}  # 空值保護
    ...
    boosted_count = (df_t['units'] == 2.0).sum()  # 觸發 2.0 unit 加倉的筆數
    ...
    return {
        'total_pnl': round(cum_pnl.iloc[-1], 2),  # 累積點數/單位
        'max_dd': round(max_dd, 2),  # 最大回撤
        'sharpe': round(sharpe, 3),  # 夏普
        'win_rate': f'{win_rate:.1%}',  # 勝率
        'trades': n,  # 總筆數
        'boosted_count': boosted_count  # 2.0 unit 加倉筆數
    }

def main():  # 主測試進入點
    df = prepare_data()  # 準備數據

    experiments = [  # 實驗配置列表 (Alpha 門檻, 加倉手數, Alpha 欄位)
        ("1. 現行基準 (加倉一律固定 1.0 unit)", None, 1.0, 'daily_alpha10_avail'),  # 基準
        ("2. Alpha10 > 2.0% 時加倉 2.0 units", 0.02, 2.0, 'daily_alpha10_avail'),  # Alpha10 > 2%
        ("3. Alpha10 > 3.0% 時加倉 2.0 units", 0.03, 2.0, 'daily_alpha10_avail'),  # Alpha10 > 3%
        ("4. Alpha10 > 4.0% 時加倉 2.0 units", 0.04, 2.0, 'daily_alpha10_avail'),  # Alpha10 > 4%
        ("5. Alpha5  > 2.0% 時加倉 2.0 units", 0.02, 2.0, 'daily_alpha5_avail'),   # Alpha5 > 2%
        ("6. Alpha5  > 3.0% 時加倉 2.0 units", 0.03, 2.0, 'daily_alpha5_avail'),   # Alpha5 > 3%
        ("7. Alpha5  > 4.0% 時加倉 2.0 units", 0.04, 2.0, 'daily_alpha5_avail'),   # Alpha5 > 4%
        ("8. 無條件加倉一律 2.0 units (固定2倍)", 0.00, 2.0, 'daily_alpha10_avail'), # 固定 2.0 units
    ]

    print("==========================================================================================")
    print(" 🧪 動態加倉手數測試: Alpha > 2%, 3%, 4% 時加倉 2.0 Units 對比 (2024/07 - 至今)")
    print("==========================================================================================")
    header = f"{'實驗方案':<45} | {'總損益 (units)':>14} | {'MDD (units)':>11} | {'Sharpe':>8} | {'勝率':>8} | {'2x加倉筆數':>10}"
    print(header)
    print("-" * len(header))

    for label, thresh, boost, col in experiments:
        t_l = run_strategy_sizing(df, is_long_only=True, alpha_threshold=thresh, boost_units=boost, alpha_col=col)  # 多單
        t_s = run_strategy_sizing(df, is_long_only=False, alpha_threshold=thresh, boost_units=boost, alpha_col=col)  # 空單
        res = eval_trades(t_l + t_s)  # 合併評估
        print(f"{label:<45} | {res['total_pnl']:>14.2f} | {res['max_dd']:>11.2f} | {res['sharpe']:>8.3f} | {res['win_rate']:>8} | {res['boosted_count']:>10}")
